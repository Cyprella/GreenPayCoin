...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a GreenPayCoin address (e.g. %1)").arg("G7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no GreenPayCoin: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert gpc:// to gpc:
    //
    //    Cannot handle this later, because gpc:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open gpc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GreenPayCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for GreenPayCoin.lnk
...
{
    return GetAutostartDir() / "gpc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a gpc.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=GreenPayCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the gpc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add gpc app to startup item list
