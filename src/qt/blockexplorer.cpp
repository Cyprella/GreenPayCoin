...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::GPC, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (gpc.conf).");
            QMessageBox::warning(this, "GreenPayCoin Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#000000; bgcolor:#4fed60;}\n a, span { font-family: monospace; }\n span.addr {color:#4fed60; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #4fed60;}\n td.d0 {font-weight: bold; color:#000000;}\n h2, h3 { white-space:nowrap; color:#4fed60;}\n a { color:#4fed60; text-decoration:none; }\n a.nav {color:#4fed60;}\n";
