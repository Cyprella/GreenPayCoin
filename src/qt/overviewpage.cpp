...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::GPC)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_GPC4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_GPC, SIGNAL(clicked()), this, SLOT(openMyAddresses()));
...
void OverviewPage::updatBlockChainInfo()
{
 if (masternodeSync.IsBlockchainSynced())
 {
    ...
    double BlockRewardGPC =  static_cast<double>(BlockReward)/static_cast<double>(COIN);
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeGreenpaycoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeGreenpaycoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeGreenpaycoinAmount = strAnonymizeGreenpaycoinAmount.remove(strAnonymizeGreenpaycoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGreenpaycoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeGreenpaycoinAmount * COIN) nMaxToAnonymize = nAnonymizeGreenpaycoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeGreenpaycoinAmount * COIN) {
   //     ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
  //                                          .arg(strAnonymizeGreenpaycoinAmount));
        strAnonymizeGreenpaycoinAmount = strAnonymizeGreenpaycoinAmount.remove(strAnonymizeGreenpaycoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGreenpaycoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeGreenpaycoinAmount == 0) {
